#K.Palof 
# ADF&G 8-3-16 updated for Seymour Canal
...
dat <- read.csv("./data/Seymour/RKCsurveyCSA_SC_15_16.csv")
                  # this is input from OceanAK - set up as red crab survey data for CSA
area <- read.csv("./data/Seymour/Seymour_strata_area.csv") 
                  #this file is the same every year.  Unless the survey methods change
histdat <- read.csv("./data/Seymour/SeymourCPUE_2015.csv")
                  ## !!!!  In future years this file will be 'JNU_CPUE_ALL' and just get updated with current years data.
females <- read.csv("./data/Seymour/SC_11-15_largefemales_bypot.csv")
...
write.csv(CPUE_wt_16, './results/Seymour/SC_CPUE_16.csv')
...
write.csv(CPUE_ALL_YEARS, './results/Seymour/Sc_perpot_all_16.csv')
...
wtd.t.test(dat5_2016$Juvenile, y = 1.83, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Small.Females, y = 1.41, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Large.Females, y = 4.45, weight = dat5_2016$weighting, samedata=FALSE)

wtd.t.test(dat5_2016$Pre_Recruit, y = 0.68, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Recruit, y = 0.68, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Post_Recruit, y = 1.45, weight = dat5_2016$weighting, samedata=FALSE)
...
dat1 %>%
  mutate(weight_lb = (exp((2.87*log(Length.Millimeters))-6.438))*(2.2/1000)) -> dat1
...
weight_lb_16
write.csv(weight_lb_16, './results/Seymour/weights_2016.csv')
##################################################################
...
poorclutch1 %>%
  mutate(y = 0, var1 = y / (y+n)) -> poorclutch1 # specifically changed for Seymour since no poor clutches were observed
poorclutch1 %>%
  group_by(Year)%>%
...
#females_16 <- females
write.csv(females_all, './results/Seymour/SC_largeF_11_16.csv')

females_all %>%
  filter(Year >=2013) -> LgF_short # short term file has last 4 years in it
#output this file as .csv to add to next year
write.csv(LgF_short, './results/Seymour/poorclutchF_ST_16.csv')
...
###################################################################
CPUE_ALL_YEARS <- read.csv("./results/Seymour/Sc_perpot_all_16.csv")
head(CPUE_ALL_YEARS)
...
# this is input from OceanAK - set up as red crab survey data for CSA
Seymour= c("Seymour Canal")
retrodata %>%
  filter(Location %in% Seymour) -> SCretrodata
SCretrodata %>%
  filter(Year > 1992) %>%
  select(-PROJECT_CO) -> SCretrodata

CPUE_ALL_YEARS %>%
  right_join(SCretrodata) -> strata79_04
...
CPUE_wt_all
write.csv(CPUE_wt_all, './results/Seymour/SC_CPUE_historical.csv')
...
CPUE_wt_79_04
write.csv(CPUE_wt_79_04, './results/Seymour/SC_CPUE_79_04.csv')
#CPUE_wt_79_04 <- read.csv("./results/Gambier/GB_CPUE_79_04.csv")
#################################################
##### graph retrospective ------------------------
#############################################
