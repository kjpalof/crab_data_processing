#K.Palof 
# ADF&G 8-3-16 updated for Pybus Bay
...
dat <- read.csv("./data/Pybus/RKCsurveyCSA_PB_15_16.csv")
                  # this is input from OceanAK - set up as red crab survey data for CSA
area <- read.csv("./data/Pybus/Pybus_strata_area.csv") 
                  #this file is the same every year.  Unless the survey methods change
histdat <- read.csv("./data/Pybus/PybusCPUE_2015.csv")
                  ## !!!!  In future years this file will be 'JNU_CPUE_ALL' and just get updated with current years data.
females <- read.csv("./data/Pybus/PB_11_15_largefemales_bypot.csv")
...
write.csv(CPUE_wt_16, './results/Pybus/PB_CPUE_16.csv')
...
write.csv(CPUE_ALL_YEARS, './results/Pybus/PB_perpot_all_16.csv')
...
wtd.t.test(dat5_2016$Juvenile, y = 1.74, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Small.Females, y = 1.09, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Large.Females, y = 3.93, weight = dat5_2016$weighting, samedata=FALSE)

wtd.t.test(dat5_2016$Pre_Recruit, y = 1.47, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Recruit, y = 1.45, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Post_Recruit, y = 2.12, weight = dat5_2016$weighting, samedata=FALSE)
...
dat1 %>%
  mutate(weight_lb = (exp((3.06*log(Length.Millimeters))-7.383))*(2.2/1000)) -> dat1
...
weight_lb_16
write.csv(weight_lb_16, './results/Pybus/weights_2016.csv')
##################################################################
...
females %>%
  mutate(Location = "Pybus Bay") -> females
females %>%
  select(Year, Location, Pot.No, n, y, var1) -> females
...
#females_16 <- females
write.csv(females_all, './results/Pybus/PB_largeF_11_16.csv')

females_all %>%
  filter(Year >=2013) -> LgF_short # short term file has last 4 years in it
#output this file as .csv to add to next year
write.csv(LgF_short, './results/Pybus/poorclutchF_ST_16.csv')
...
###################################################################
CPUE_ALL_YEARS <- read.csv("./results/Pybus/PB_perpot_all_16.csv")
head(CPUE_ALL_YEARS)
...
pybus= c("Pybus Bay")
retrodata %>%
  filter(Location %in% pybus) -> PBretrodata
PBretrodata %>%
  filter(Year > 1992) %>%
  select(-PROJECT_CO) -> PBretrodata

CPUE_ALL_YEARS %>%
  right_join(PBretrodata) -> strata79_04
...
CPUE_wt_all
write.csv(CPUE_wt_all, './results/Pybus/PB_CPUE_historical.csv')
...
CPUE_wt_79_04
write.csv(CPUE_wt_79_04, './results/Pybus/PB_CPUE_79_04.csv')
