#K.Palof 
# ADF&G 8-4-16 updated for Peril Strait
...
dat <- read.csv("./data/Peril/RKCsurveyCSA_PS_15_16.csv")
                  # this is input from OceanAK - set up as red crab survey data for CSA
area <- read.csv("./data/Peril/Peril_strata_area.csv") 
                  #this file is the same every year.  Unless the survey methods change
histdat <- read.csv("./data/Peril/PerilCPUE_2015.csv")
                  ## !!!!  In future years this file will be 'JNU_CPUE_ALL' and just get updated with current years data.
females <- read.csv("./data/Peril/PS_11_15_largefemales_bypot.csv")
...
write.csv(CPUE_wt_16, './results/Peril/PS_CPUE_16.csv')
...
write.csv(CPUE_ALL_YEARS, './results/Peril/PS_perpot_all_16.csv')
...
wtd.t.test(dat5_2016$Juvenile, y = 5.59, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Small.Females, y = 7.46, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Large.Females, y = 6.66, weight = dat5_2016$weighting, samedata=FALSE)

wtd.t.test(dat5_2016$Pre_Recruit, y = 1.85, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Recruit, y = 0.86, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Post_Recruit, y = 0.62, weight = dat5_2016$weighting, samedata=FALSE)
...
dat1 %>%
  mutate(weight_lb = (exp((3.02*log(Length.Millimeters))-7.18))*(2.2/1000)) -> dat1
...
weight_lb_16
write.csv(weight_lb_16, './results/Peril/weights_2016.csv')
##################################################################
...
#females_16 <- females
write.csv(females_all, './results/Peril/PS_largeF_11_16.csv')

females_all %>%
  filter(Year >=2013) -> LgF_short # short term file has last 4 years in it
#output this file as .csv to add to next year
write.csv(LgF_short, './results/Peril/poorclutchF_ST_16.csv')
...
###################################################################
CPUE_ALL_YEARS <- read.csv("./results/Peril/PS_perpot_all_16.csv")
head(CPUE_ALL_YEARS)
...
# this is input from OceanAK - set up as red crab survey data for CSA
Peril= c("Deadman Reach")
retrodata %>%
  filter(Location %in% Peril) -> PSretrodata
PSretrodata %>%
  filter(Year > 1992) %>%
  select(-PROJECT_CO) -> PSretrodata

CPUE_ALL_YEARS %>%
  right_join(PSretrodata) -> strata79_04
...
CPUE_wt_all
write.csv(CPUE_wt_all, './results/Peril/PS_CPUE_historical.csv')
...
CPUE_wt_79_04
write.csv(CPUE_wt_79_04, './results/Peril/PS_CPUE_79_04.csv')
