#K.Palof 
# ADF&G 8-12-16 updated for Gambier
...
dat <- read.csv("./data/Gambier/RKCsurveyCSA_GB_15_16.csv")
                  # this is input from OceanAK - set up as red crab survey data for CSA
area <- read.csv("./data/Gambier/Gambier_strata_area.csv") 
                  #this file is the same every year.  Unless the survey methods change
histdat <- read.csv("./data/Gambier/GambierCPUE_2015.csv")
                  ## !!!!  In future years this file will be 'JNU_CPUE_ALL' and just get updated with current years data.
females <- read.csv("./data/Gambier/GB_11_15_Large females_bypot.csv")
...
write.csv(CPUE_wt_16, './results/Gambier/GB_CPUE_16.csv')
...
write.csv(CPUE_ALL_YEARS, './results/Gambier/GB_perpot_all_16.csv')
...
wtd.t.test(dat5_2016$Juvenile, y = 3.88, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Small.Females, y = 3.37, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Large.Females, y = 5.17, weight = dat5_2016$weighting, samedata=FALSE)

wtd.t.test(dat5_2016$Pre_Recruit, y = 2.26, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Recruit, y = 1.73, weight = dat5_2016$weighting, samedata=FALSE)
wtd.t.test(dat5_2016$Post_Recruit, y = 1.85, weight = dat5_2016$weighting, samedata=FALSE)
...
dat1 %>%
  mutate(weight_lb = (exp((2.921*log(Length.Millimeters))-6.695))*(2.2/1000)) -> dat1
...
weight_lb_16
write.csv(weight_lb_16, './results/Gambier/weights_2016.csv')
##################################################################
...
#females_16 <- females
write.csv(females_all, './results/Gambier/GB_largeF_11_16.csv')

females_all %>%
  filter(Year >=2013) -> LgF_short # short term file has last 4 years in it
#output this file as .csv to add to next year
write.csv(LgF_short, './results/Gambier/poorclutchF_ST_16.csv')
...
###################################################################
CPUE_ALL_YEARS <- read.csv("./results/Gambier/GB_perpot_all_16.csv")
head(CPUE_ALL_YEARS)
...
# this is input from OceanAK - set up as red crab survey data for CSA
Gambier= c("Gambier Bay")
retrodata %>%
  filter(Location %in% Gambier) -> GBretrodata
GBretrodata %>%
  filter(Year > 1992) %>%
  select(-PROJECT_CO) -> GBretrodata

CPUE_ALL_YEARS %>%
  right_join(GBretrodata) -> strata79_04
...
CPUE_wt_all
write.csv(CPUE_wt_all, './results/Gambier/GB_CPUE_historical.csv')
...
CPUE_wt_79_04
write.csv(CPUE_wt_79_04, './results/Gambier/GB_CPUE_79_04.csv')
